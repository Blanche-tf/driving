...
//  }
  ...
  $.extend(prototype, {
    init: function () {
      ...
      options.url = 'data-view';

      $(this.$element).delegate('img', 'mouseover', function() {
        
        if (this.className.indexOf('viewer-toggle') >= 0) {
          return;
        }

        if (this.getAttribute('no')!==null) {
          return;
        }
        ...
        $this.off(EVENT_CLICK).on(EVENT_CLICK, $.proxy(me.start, me));
      });
      ...
      if (options.inline) {
        ...
        $this.off(EVENT_CLICK).on(EVENT_CLICK, $.proxy(this.start, this));
      }
    },
  ...
  $.extend(prototype, {
    build: function () {
      ...
      this.$canvas = $canvas = $viewer.find('.viewer-canvas');
  ...
  $.extend(prototype, {
    ...
    initList: function () {
      ...
      this.$images.each(function (i) {
        var src = this.getAttribute('data-view') || this.src;
  ...
  $.extend(prototype, {
    ...
    show: function (index) {
      ...
      this.$body && this.$body.addClass(CLASS_OPEN);
      ...
      this.$element.one(EVENT_SHOWN, $.proxy(function () {
        ...
        this.view(index || (this.target ? this.$images.index(this.target) : 0));
    ...
    // Reset the image to its initial static.
    reset: function () {
      if (this.isShown && !this.isPlayed) {
